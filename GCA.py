...
#from MAResUNet import PAM_CAM_Layer
#from MANet import PAM_CAM_Layer
from PSA import PSA_s
from PSA import PSA_p
from MACUNet import ACBlock
...
class ResNet(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        return x, low_level_feat, low_level_feat1
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=6, os=16, pretrained=False, _print=True):
        ...
        self.attention = PSA_s(256,256)
        self.attention1 = PSA_p(256,256)
        ...
        channels = [256, 48, 64, 128, 256, 512]
        self.conv66 = nn.Sequential(
            ACBlock(channels[5], channels[0]),
    ...
    def forward(self, input):
        x, low_level_features, low_level_feat1 = self.resnet_features(input)
        x1 = self.aspp1(x)
        Wv1 = x1
        
        x1 = self.attention(x1, Wv1)
        
        x2 = self.aspp2(x)
        x2 = self.attention(x2, x1)
        
        x3 = self.aspp3(x)
        x3 = self.attention(x3, x2)
        
        x4 = self.aspp4(x)
        x4 = self.attention(x4, x3)
        
        x5 = self.global_avg_pool(x)
        x5 = F.interpolate(x5, size=x4.size()[2:], mode='bilinear', align_corners=True)
     #   x5 = F.upsample(x5, size=x4.size()[2:], mode='bilinear', align_corners=True)
     
   #     low_level_feat1 = self.conv66(low_level_feat1)
  #      low_level_feat1 = self.bn3(low_level_feat1)
  #      low_level_feat1 = self.relu(low_level_feat1)
     

  #      x = torch.cat((x1, x2, x3, x4, x5, low_level_feat1), dim=1)
 ...
 #       x = F.upsample(x, size=(int(math.ceil(input.size()[-2]/4)),
  ...
  #      low_level_features = self.conv66(low_level_features)
  #      low_level_features = self.bn3(low_level_features)
  #      low_level_features = self.relu(low_level_features)
  #      low_level_features = self.conv67(low_level_features)
        low_level_features = self.conv68(low_level_features)
...
def get_10x_lr_params(model):
    ...
    """
    b = [model.aspp1, model.aspp2, model.aspp3, model.aspp4, model.conv1, model.conv2, model.last_conv, model.conv66, model.conv67, model.conv68]
