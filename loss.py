...
def softmax_focalloss(outputs, labels, ignore_index=255, gamma=2.0, normalize=False):
    """
    ...
    """
    losses = F.cross_entropy(outputs, labels, ignore_index=ignore_index, reduction='none')
    with torch.no_grad():
        p = outputs.softmax(dim=1)
        modulating_factor = (1 - p).pow(gamma)
        valid_mask = ~ labels.eq(ignore_index)
        masked_labels = torch.where(valid_mask, labels, torch.zeros_like(labels))
        modulating_factor = torch.gather(modulating_factor, dim=1, index=masked_labels.unsqueeze(dim=1)).squeeze_(dim=1)
        scale = 1.
        if normalize:
